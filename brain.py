...
class Brain:
    ...
    def __init__(self, inputs, clone=False):
        ...
        self.nodes = []
        self.inputs = inputs
        # list called Nets and this list is going to store the nodes that appear in the
        # neural net ordered by the layer that the nodes appear in So at the beginning of this list we'll have all the nodes that
        # appear in the input layer and at the end of this list we're going to have the one node that comes in the output layer
        self.net = []
        self.layers = 2
